...
public:
    ToastDlg()
    {
        ...
        setStyleSheet("color: #fff;background:#439fb6; border-radius:50px;font-size:50px;padding:20px;");
    ...
    void show(Toast::Level level, const QString& text)
    {
        ...
        if (level == Toast::INFOTYPE)
        {
            p.setColor(QPalette::Window, QColor(0,0,0,200));//黑色
        }
        else if (level == Toast::WARNTYPE)
        {
        ...
        QDialog::show();
        this->move(int((1920-this->width())/2),100);
    }
};//~class ToastDlg end
